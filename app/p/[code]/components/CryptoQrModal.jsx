...
export default function CryptoQrModal({ open, onClose, address, amount, networkName, hint, messages }) {
    if (!open) return null;
    return (
        <div role="dialog" aria-modal="true"
            ...
            <div onClick={(e)=>e.stopPropagation()}
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <h3 className="card-title" style={{ margin:0 }}>{messages.cryptoModalTitle}</h3>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:12, alignItems:'center', marginTop:12 }}>
                    ...
                    <div style={{ textAlign:'center', width:'100%' }}>
                        <div className="label" style={{ marginBottom:6 }}>{messages.network}</div>
                        <div style={{ fontWeight:800, marginBottom:10 }}>{networkName || '—'}</div>

                        <div className="label" style={{ marginBottom:6 }}>{messages.address}</div>
